...
const selectRecord = async (param, callback) => {
    ...
    //     return callback(null, ans)
    // })
    let sql = query.optimize.rec(table.returned, options.filter, options.order)
...
const uniqueRecord = async (param, callback) => {
    ...
    //     return callback(null, ans)
    // })
    let sql = query.optimize.get(table.returned, table.returned.fields.id)
...
const searchRecord = async (param, callback) => {
    ...
    //     return callback(null, ans)
    // })
    let sql = query.optimize.src(table.returned, helper.filters, [id?.Asc()])
...
const inventoryRecord = async (param, callback) => {
    let { item, id } = table.returned.fields
    let options = {
